            ...
            with tf.device('/gpu:%d' % gpu_index):
                ...
                with tf.name_scope('%s_%d' % ('gpu', gpu_index)) as scope:
                    videos_degraded = residualNet(
                        videos_placeholder[gpu_index * cfg['BATCH_SIZE']:(gpu_index + 1) * cfg['BATCH_SIZE']])
                    if cfg['USE_INSTANCE_NORM']:
                        videos_degraded = _instance_norm(videos_degraded)
                    if cfg['USE_BILINEAR_RESIZE']:
                        videos_degraded = _bilinear_resize(videos_degraded)
                    if cfg['USE_BINARIZE']:
                        videos_degraded = tf.reduce_mean(videos_degraded, axis=4, keep_dims=True)
                        videos_degraded = tf.nn.relu(tf.sign(videos_degraded))
                        videos_degraded = tf.tile(videos_degraded, [1, 1, 1, 1, 3])
...
def main(_):
    checkpoint_dir = '/hdd1/wuzhenyu_sjtu/checkpoint_UseResidual/L1Loss_NoLambdaDecay_AvgReplicate_MonitorBudget_MonitorUtility_Resample_18_2.0_0.5'
