...
class AdversarialTraining(object):
    ...
    '''
    def create_architecture_adversarial(self, scope, videos, utility_labels, budget_labels, dropout, is_training, lambda_):
        degrad_videos = residualNet(videos, is_video=True)
        ...
        logits_utility = utilityNet(degrad_videos, dropout, wd=0.001)
        loss_utility = tower_loss_xentropy_sparse(scope, logits_utility, utility_labels, use_weight_decay=True)
        logits_budget = tf.zeros([self.cfg['TRAIN']['BATCH_SIZE'], self.cfg['DATA']['NUM_CLASSES_BUDGET']])
        loss_budget = 0.0
        logits_lst = []
                ...
                for gpu_index in range(0, self.cfg['TRAIN']['GPU_NUM']):
                    with tf.device('/gpu:%d' % gpu_index):
                        ...
                        with tf.name_scope('%s_%d' % ('gpu', gpu_index)) as scope:
                            ...
                            loss_degrad, loss_budget, loss_utility, logits_budget, logits_utility, argmax_adverse_budget_loss = self.create_architecture_adversarial(scope, videos, utility_labels, budget_labels, dropout_placeholder, is_training_placeholder, _lambda_op)
