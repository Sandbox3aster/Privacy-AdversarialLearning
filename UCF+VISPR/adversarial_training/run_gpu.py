...
def run_training_multi_model():
    ...
    logits_budget_videos_lst_dct = defaultdict(list)
    loss_budget_videos_lst_dct = defaultdict(list)
    ...
    with tf.Graph().as_default():
        with tf.Session(config=config) as sess:
            ...
            #budgetNet_model_name_lst = ['resnet_v1_50', 'resnet_v2_50', 'mobilenet_v1', 'mobilenet_v1_075']
            #budgetNet_model_name_lst = ['resnet_v1_50', 'resnet_v2_50', 'mobilenet_v1']
            #budgetNet_model_name_lst = ['mobilenet_v1', 'mobilenet_v1_075']
            budgetNet_model_name_lst = ['mobilenet_v1']
            #budgetNet_model_name_lst = ['resnet_v1_50']
            budgetNet_dict = {}
            for model_name in budgetNet_model_name_lst:
                budgetNet_dict[model_name] = nets_factory.get_network_fn(
            ...
            if use_pretrained_model:
                ...
                saver = tf.train.Saver(varlist_utility + varlist_degradtion)
                ckpt = tf.train.get_checkpoint_state(checkpoint_dir=FLAGS.whole_pretrained_checkpoint_dir)
                if ckpt and ckpt.model_checkpoint_path:
                    saver.restore(sess, ckpt.model_checkpoint_path)
                    print('Session restored from trained model at {}!'.format(ckpt.model_checkpoint_path))
            ...
            for step in xrange(FLAGS.max_steps):
                ...
                for _ in itertools.repeat(None, 5):
                    # for _ in itertools.repeat(None, FLAGS.adaptation_budget_steps):
                    start_time = time.time()
                    sess.run(zero_ops_budget)
                    loss_budget_lst = []
                    for _ in itertools.repeat(None, FLAGS.n_minibatches):
                ...
                if step != 0 and FLAGS.use_resampling and step % FLAGS.resample_step == 0:
                    saver = tf.train.Saver(varlist_budget)
                    ckpt = tf.train.get_checkpoint_state(checkpoint_dir=FLAGS.budget_multi_models)
                    if ckpt and ckpt.model_checkpoint_path:
                        saver.restore(sess, ckpt.model_checkpoint_path)
        ...
        with tf.Session(config=config) as sess:
            ...
            try:
                while not coord.should_stop():
                    ...
                    logits_budget, loss_budget = sess.run([logits_budget_images_op, loss_budget_op],
                                feed_dict={images_placeholder: images,
                                           budget_images_labels_placeholder: images_labels,
                                           dropout_placeholder: 1.0,
                                           isTraining_placeholder: True})
...
def plot_heatmap(vid, vid_orig, heatmap_dir):
    ...
    print(vid.shape)
    print(vid_orig.shape)
    for i in range(vid.shape[0]):
        frame_downsample_crop = crop_center(vid_orig[i],112,112)
        frame = vid[i]
        frame = crop_center(frame, 112, 112)
        plt.subplot(2, 3, 1)
        #frame_downsample = imresize(vid_orig[i], scalar_scale=1.0/4.0).astype('uint8')
        plt.imshow(frame_downsample_crop.astype('uint8'))
        plt.axis('off')
        plt.title('Dowmsampled')
        frame_scaled = linear_scaling(frame, vid, video_linear_scaling=True, joint_channels=True).astype('uint8')
        plt.subplot(2, 3, 2)
        plt.imshow(frame_scaled)
        plt.axis('off')
        plt.title('Joint Scaling')
        frame_scaled = linear_scaling(frame, vid, video_linear_scaling=True, joint_channels=False).astype('uint8')
        plt.subplot(2, 3, 3)
        plt.imshow(frame_scaled)
        plt.axis('off')
        plt.title('Separate Scaling')
        for j in range(frame.shape[2]):
            plt.subplot(2,3,j+4)
            heatmap = plt.imshow(frame[:,:,j])
            heatmap.set_cmap('jet')
            plt.colorbar()
            plt.axis('off')
            plt.title('Channel{}'.format(j))
        plt.savefig('{}/heatmap_{}.png'.format(heatmap_dir, i))
        plt.close()

def plot_visualization(vid, vid_orig, heatmap_dir):
    def crop_center(img, cropx, cropy):
        y, x,_ = img.shape
        startx = x // 2 - (cropx // 2)
        starty = y // 2 - (cropy // 2)
    ...
    for j in range(vid.shape[0]):
        ...
        for i in range(1, 3):
            axes[0, i] = fig.add_subplot(4, 3, 1 + i, sharex=axes[0, 0], sharey=axes[0, 0])
        for i in range(3, 12):
            axes[i // 3, i % 3] = fig.add_subplot(4, 3, 1 + i)
        ...
        for k in range(frame.shape[2]):
            axes[3, k].imshow(frame[:,:,k], cmap=plt.get_cmap('jet'))
            axes[3, k].set_title('Channel{}'.format(j))
        # prevent overlap of y-axis labels
        fig.tight_layout()
        plt.savefig('{}/vis_{}.png'.format(heatmap_dir, j))
        plt.close()
        video.append(img_eq)
    return video
...
def write_video(directory, X, X_orig, Y):
            ...
            with tf.device('/gpu:%d' % gpu_index):
                ...
                with tf.name_scope('%s_%d' % ('gpu', gpu_index)) as scope:
                    videos = tf.reshape(
                        ...
                        [FLAGS.video_batch_size * FLAGS.depth, FLAGS.crop_height, FLAGS.crop_width, FLAGS.nchannel])
                    with tf.variable_scope(tf.get_variable_scope(), reuse=False):
                        videos_degraded = residualNet(videos)
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                write_video(directory, videos_degraded_value, videos, labels)
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                write_video(directory, images_degraded_value, images, images_labels)
                #raise tf.errors.OutOfRangeError
        except tf.errors.OutOfRangeError:
            print('Done testing on all the examples')
        finally:
